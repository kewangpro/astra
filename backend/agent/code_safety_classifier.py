...
class CodeSafetyClassifier:
    ...
    def _static_check(script: str) -> SafetyVerdict:
        ...
        if (
            ...
            and re.search(r"dpo_train\.py|grpo_train\.py|distill_train\.py|rft_train\.py|bare_eval\.py|sft_train\.py|star_train\.py", script)
        ):
            return SafetyVerdict(
                safe=True,
                reason="os.execv dispatch to a known finetune/eval script (dpo/grpo/distill/rft/sft/star_train.py, bare_eval.py) with no network calls — auto-approved",
                classifier="static",
            )
