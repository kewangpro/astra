...
- **Pivot Engine** (`backend/loop/pivots.py`): Detects plateaus and drives escalating pivot strategy.
  ...
  - **Forced real algorithm switch** (`_resolve_pivot_algorithm` / `_pick_switch_algorithm`): on vanilla SB3 RL (no `trainer_type`) at escalation ≥ 2, if the mission has never actually changed trainer (`all_algos_tried` empty / only the current canonical name) and the LLM proposed the same trainer under an alias or omitted `algorithm`, the loop picks a different discrete SB3 algo (recipe algorithm first, then DQN / PPO / A2C). Custom trainers (`actor_critic`, `lookahead_*`) are not rewritten. Applied trainer names are persisted in `plan["all_algos_tried"]`. Live-verified on `7c567090` iter 54: Level 3 Snake `env_kwargs` proposal still became `algo: PPO→DQN`, and `train.py` dispatched `stable_baselines3.DQN` with Seaquest recipe HPs. `env_kwargs` from earlier reward-shaping pivots are **not** cleared on an algo switch.
