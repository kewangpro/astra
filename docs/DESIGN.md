...
- **Pivot Engine** (`backend/loop/pivots.py`): Detects plateaus and drives escalating pivot strategy.
  ...
  - **Post-pivot regression detection**: arch and algorithm pivots force a fresh training start (no warm-start, reset `best_score.txt`), which can cause temporary catastrophic regression. Before applying any arch/algo pivot the state machine: (1) backs up `best_model.zip` → `best_model_pre_pivot.zip`; (2) saves a `_pre_pivot_hps` snapshot in the plan dict; (3) calls `pivot_engine.record_arch_pivot_baseline()` to arm the regression detector. After each subsequent iteration, `should_revert_pivot()` compares the post-pivot best against the pre-pivot best; if the new config is still > 20% (`PIVOT_REGRESSION_THRESHOLD`) below the baseline after `PLATEAU_WINDOW=3` iters, the state machine restores the checkpoint, restores the HPs, calls `revert_escalation()` (decrements `_pivot_count` by 1), and clears the regression window so HP-level tuning resumes from the good baseline. If the new config recovers within 3 iters, the tracking is cleared silently without reverting.
