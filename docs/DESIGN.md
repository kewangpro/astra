...
2. **Sandbox Handling**: The Mission Store tracks either a `ContainerID` (cloud/CPU) or a `SubprocessPID` (Apple Silicon GPU). `SandboxManager.recover()` checks whether the sandbox is still alive. **If alive ("reattached")**: the sandbox is terminated via `SandboxManager.terminate()` — the stale subprocess would be unsupervised (no evaluation or pivoting loop), so it is safer to restart cleanly from the last checkpoint. **If gone ("reset")**: no action needed. All recovered missions are reset to `PENDING` and their `container_id` / `subprocess_pid` cleared.
