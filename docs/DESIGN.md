...
- **Pivot Engine** (`backend/loop/pivots.py`): Detects plateaus and drives escalating pivot strategy.
  ...
  - **Architecture ladder (`_gate_arch_proposal`, Phase 75)**: Level 0 drops `policy_kwargs` even if the LLM proposes `net_arch`. Levels 1–3 refuse a strictly smaller list-style net than `best_policy_kwargs` (else current). Level 4 (force novel) may shrink. Real incident: locked DQN-70 `601c2404` applied `[128]` at `esc=1` over a 43-score `[400,300]`.
