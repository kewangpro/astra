...
**MinAtar Asterix & Grid Pac-Man (Phase 81):** `MinAtarAsterixEnv` follows kenjyoung `asterix.py` (player rows 1–8, original `n/l/u/r/d/f` action order, gold vs enemies, trail heading, ramping spawn/move). `GridPacManEnv` is an ASTRA 10×10 maze (pellets, power, four ghosts). Domain matching uses `asterix` before `asteroid` so play/tournament do not load Asteroids for Asterix models.
