...
- [x] **`_seed_plan_algorithm()`** — on a fresh plan only (iter 0 / critic revise, not resume/pivot), unnamed RL missions overwrite `plan["algorithm"]` from `_load_recipe_for_env(env_id)["algorithm"]` when the recipe declares one. Named goals win over the recipe. Non-RL plans are only canonicalized.
