...
- [x] **Metric Gap now tracks the same numbers as pass_rate history, live** — a mission whose training crashed (the `UnicodeDecodeError` above) or otherwise failed before reaching the post-training `_run_bare_eval` check previously showed `best_metric_value=null` / "0.0% — NO METRICS" even when the live-tailed pass_rate history clearly showed real progress (e.g. 74.2%). New `LoopStateMachine._update_live_pass_rate(mission_id, value, iteration)` is called from `_tail_remote_metrics()` on every parsed `Pass rate:` line — it always updates `Mission.current_metric_value`, and updates `best_metric_value`/`best_metric_iteration` only if the new reading is higher than the current best. `_run_bare_eval`'s post-training result can still overwrite these with its own more-rigorous check (per `FINETUNE.md`'s note that quicker eval methods are "saturated" and less discriminating) — this is a fallback that keeps the HUD honest in between, not a replacement. 3 new tests in `test_remote_telemetry_tail.py` (including a real in-memory-DB test verifying best-tracking keeps the higher earlier value when a later reading regresses).
...
- [x] **`MissionUpdate` schema accepts `best_metric_iteration`/`current_metric_value`** (`schemas/mission.py`) — the `PATCH /missions/{id}` endpoint previously only accepted `best_metric_value`, with no way to backfill the other two Metric-Gap-related fields without direct DB access. Used to backfill mission `e16f4f37`'s stale `null` fields (from before the live-tracking fix above existed) with its actual observed values (`best_metric_value=0.742`, `current_metric_value=0.697`) via the API rather than a raw SQL update.
    ...
    Total: **670 tests** (661 unit + 9 integration).
