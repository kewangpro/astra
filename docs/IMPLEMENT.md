...
- [x] **11 new/updated tests**: `test_state_recovery.py` rewritten for the new dict return and the "alive is never terminated, for any backend" rule (`test_reattached_mission_resumes_in_place`, `test_remote_pid_passed_to_recover_and_kept_on_reattach`, `test_mixed_outcomes_split_between_resume_and_restart`, others updated). `test_subprocess_sandbox.py` — 2 tests for `is_alive()` via `_reattach_pid`. `test_sandbox_manager.py` — 3 tests (container reattach registers a sandbox, container recover-dead, SSH reattach syncs tail offset to remote log size). `test_ssh_sandbox.py` — 2 tests for `sync_tail_offset_to_current()`. `test_loop_state_machine.py` — `test_resume_existing_sandbox_skips_launch_and_reuses_saved_plan` (asserts `launch()` never called, approval gate never requested, mission still reaches `COMPLETED`).

    Total: **700 tests** (689 unit + 11 integration).
...
- [x] **2 new tests** (`test_code_generator.py`): `test_resolve_hyperparams_dpo_ignores_plan_override` and `test_resolve_hyperparams_grpo_ignores_plan_override` — assert a plan-provided `learning_rate`/`num_layers` has zero effect on the resolved hyperparameters for these task types.
    ...
    Total: **702 tests** (691 unit + 11 integration).
