...
- [x] **Live-verified** on mission `7c567090` (unnamed Seaquest, target 50, uvicorn running the new code): after iter 54 the resolver emitted `algo: PPO→DQN` despite a Level 3 Snake `env_kwargs` proposal; `train.py` dispatched `stable_baselines3.DQN` with Seaquest recipe HPs (`lr=2.5e-4`, `buffer_size=1e5`, `total_timesteps=300000`); PPO-checkpoint warm-start failed the DQN load and started fresh. `env_kwargs.death_penalty=-5` from earlier reward-shaping pivots was **not** cleared (algo switch only replaces hyperparameters). Stall clock was already `pivot_count=13` / best 21.5 @ iter 6 — the DQN try has a short window to post a new best before `is_converged()` at count 15.
