...
- [x] **4 new tests**: `test_ssh_sandbox.py` — `is_alive` inconclusive-output case (asserts `True`, no `_sync_back`), `terminate` unreachable-host case (asserts no `STOPPED`/`_sync_back`, `remote_pid` retained); existing happy-path `terminate` tests updated to mock `returncode=0`. `test_loop_state_machine.py` — `test_launch_failure_terminates_sandbox_before_marking_failed` (launch() raises mid-loop → asserts both `FAILED` status and `terminate()` called).
    ...
    Total: **692 tests** (682 unit + 10 integration).

---

## Phase 27 — Sandbox Reattach: Resume In Place Instead of Kill-and-Restart
