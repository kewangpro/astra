...
- [x] ~~Static auto-approve short-circuit for the dpo/grpo dispatch wrapper~~ **— documented here as done, but the actual check was never added to `_static_check()`; see Phase 28, where it was actually implemented after a live incident exposed the gap.** The intent was: alongside the existing "all requests target localhost" short-circuit, if the script has no `requests` calls *and* contains `os.execv(...)` targeting `dpo_train.py`/`grpo_train.py` by name, auto-approve statically without ever reaching the LLM classifier. This is exactly and only the shape `_DPO_TEMPLATE`/`_GRPO_TEMPLATE` produce — `os.execv` here is legitimate process-image replacement (see the orphan-proofing fix above), not arbitrary code execution. Only the LLM-prompt-clarification half of the fix (below) actually shipped in this phase.
...
**Follow-up problem: a third backend crash, from the same class of bug, at a Metal-thread-safety entry point the earlier fixes didn't cover.** Signature this time: `-[AGXG16GFamilyCommandBuffer ...]: failed assertion 'A command encoder is already encoding to this command buffer'` — not the OOM class the two earlier crashes were (`ModelManager.before_sandbox_launch()`, `MLXProvider.load()`), but a genuine concurrency violation: two Metal operations encoding to the same command buffer at once. `MLXProvider.generate()`/`load()` already serialized themselves via a module-local `_MLX_LOCK` (with the comment "concurrent Metal GPU access causes SIGABRT" — the exact mechanism, already known), but `MLXProvider.unload()` and `ModelManager._gc()` both called `mx.metal.clear_cache()` directly, completely unprotected by that lock. `generate()`'s actual model-forward-pass work runs in a background thread via `run_in_executor` (held across the lock's duration) — genuinely concurrent with the main event-loop thread, where an unlocked `_gc()`/`unload()` cache-clear could fire at the same real instant.
...
- [x] **9 new/updated tests** (`test_tetris_env.py`): observation shape updated from `(4,)` to `(18,)` everywhere it was asserted; new tests for current/next piece one-hot correctness, piece encoding updating correctly after a step, and `get_next_states()` staying 4-feature independent of `_obs()`'s new shape.
    ...
    Note: **live-verified against the actual stalled DQN mission (`fa203f6b`, resumed after this fix).** `env.observation_space` confirmed `Box(0.0, 1.0, (18,), float32)` in the sandbox log; the mission's 124-iteration-old checkpoint (built for the old `(4,)` shape) correctly failed SB3's own `check_for_correct_spaces` validation on warm-start load — already wrapped in the existing `except Exception` guard (`code_generator.py`'s warm-start block, Phase 27 above) — logged a warning and safely fell back to a freshly-initialized model instead of crashing. Early signal post-fix: `mean_reward` starting near 15, above the pre-fix plateau band of 9–11, consistent with the piece-identity theory (not yet confirmed against `lines_cleared` over a full run).
...
- [x] **New `LoopStateMachine._lookahead_trainer_type_for()` classmethod** — when an explicit DQN/PPO/A2C request is made for Tetris-v0 (goal-text-locked, same detection as `_is_algorithm_locked()`), routes `plan["trainer_type"]` to `lookahead_dqn`/`lookahead_ppo`/`lookahead_a2c` instead of the old `"sb3"` sentinel that just forced vanilla SB3. This still honors "asked to use DQN, use DQN" (Phase 27's governing principle) — the algorithm choice is preserved, it's no longer structurally hopeless. Extracted from `run()`'s inline `elif` into a standalone, directly testable classmethod (mirroring `_should_force_actor_critic`'s existing pattern).
...
## Phase 30 — Snake-v0 Flood-Fill Reachable-Space Feature
