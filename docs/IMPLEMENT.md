...
## Phase 36 — DPO Recipe's Learning Rate Retuned: Correctly Safe, But Too Weak To Learn Anything

**Problem:** Phase 35's checkpoint-chaining fix was necessary but not sufficient — 22+ iterations after it shipped, mission `7fd88324` still hadn't beaten iteration 39's 0.818, because nothing had produced a new best for it to chain from. Live-verified root cause via three manual, offline diagnostics on `mac-mini.local` (outside the mission loop, cancelling the live mission first to free the GPU, each reusing the exact same collected preference pairs for a clean apples-to-apples comparison):

1. **`learning_rate=5e-7` (the recipe's prior value):** per-epoch avg_loss barely moved (~0.693 → ~0.676 over a full 3-epoch run) — and this pattern held nearly identically across 15+ different iterations with completely different collected pairs each time. The recipe wasn't failing to find a good config via search; the actual gradient step size was too small to produce any signal differentiated by the data at all, regardless of what pivots did to `temp`/`k_collect`.
2. **`learning_rate=5e-6` (10x):** catastrophic collapse — 0% pass rate (66/66 failures) by step ~56, wildly unstable per-step loss (0.0087 ↔ 1.5338). This is the exact failure signature of the original Phase 26 incident (`learning_rate=0.001` collapsing pass_rate from 62%→0% within 50 steps) — confirms the recipe-lock guard is genuinely load-bearing, not overcautious.
...
## Phase 37 — Generated os.execv(*argv) Star-Unpack Silently Wasted Iterations

**Problem:** live-verified — mission `7fd88324` occasionally showed an "instant complete" pattern (a normal iteration takes 90-150+ min; these finished in under 2 minutes) reporting a score identical to the immediately prior iteration. Happened at least 4 times across the mission's history (iterations ~152/153, 168, 185) before being root-caused. The remote wrapper script's own traceback, caught live for iteration 185: `os.execv(*argv)` / `TypeError: execv expected 2 arguments, got 41` — the LLM generating that iteration's `train.py` wrapper had written `os.execv(*argv)`, star-unpacking a ~40-element argv list into that many positional arguments, instead of the `_DPO_TEMPLATE`/`_GRPO_TEMPLATE`-specified `os.execv(path, argv_list)` (2 arguments). `os.execv()` raises `TypeError` immediately on the wrong arity — before `dpo_train.py`/`grpo_train.py` ever runs. The wrapper process then exits near-instantly; `SandboxManager` sees the tracked pid gone and treats the sandbox as "done"; the evaluator falls back to re-scoring the previous iteration's stale checkpoint, producing a duplicate score. That duplicate score then fed directly into `PivotEngine`'s plateau detection as if it were a real, redundant data point — an entire wasted iteration disguised as a normal (if unimproving) one.
...
## Phase 38 — Crystallizer Dumped DPO/GRPO's Fictional Plan Instead of the Real Recipe

**Problem:** live-verified — mission `43517dd5` completed successfully (pass_rate 0.833, beating its 0.8 target) and got auto-crystallized into `dpo_dpo_v1.yaml`. The resulting recipe was badly wrong: `learning_rate: 0.001` (the real run used `2e-6` — off by 500x), `num_layers: 5` (real run used `8`), plus invented fields that don't exist in `dpo_train.py`'s actual CLI at all (`batch_size`, `iters`, `mask_prompt`, unfilled placeholder strings like `warm_start_adapter_path: "/path/to/adapter"`) and a fabricated 3-phase `curriculum` block (`dpo_train.py` has no phased-training concept whatsoever). Root cause: `crystallizer.py`'s `_build_recipe_content()` only has hyperparameter-cleaning logic for `task_type == "rl"` (`_clean_rl_hyperparams`, itself added after an earlier documented incident of this same failure class for RL). Every other task type — including dpo/grpo — just dumps `plan.hyperparameters` verbatim. For dpo/grpo specifically that's not a minor omission: `code_generator.py`'s `_resolve_hyperparams()` deliberately discards nearly all plan-proposed dpo/grpo hyperparameters at dispatch time (recipe-authoritative, full stop — Phase 26), so `plan.hyperparameters` for these task types is *always* the LeadAgent's discarded, never-actually-used proposal, not what trained the model. Confirmed directly via `GET /agent/missions/{id}/plan` on the live mission — the stored plan hyperparameters matched the bogus crystallized recipe exactly.
...
## Phase 39 — Metric History Chart Collapsed dpo Iterations Onto One Point

**Problem:** live-verified on a running dpo mission — the "Metric History" chart in the Mission HUD (`MetricChart.tsx`) showed a single lone data point no matter how many iterations had run. Root cause: run-boundary detection inferred a new run whenever the step counter *decreased* (`curr < prev`) — correct for RL's per-timestep counter, which climbs within a run and resets at the next one's start, but wrong for dpo/grpo's `loss`, which `dpo_train.py` logs once per epoch. Under the `epochs=1` recipe (Phase 36), every single iteration's one loss point sits at `step=1`, so `1 < 1` never fires — every iteration was silently treated as a continuation of the same run, each one overwriting the last at the identical x-coordinate.
...
- [x] **Not a bug, confirmed separately**: dpo iterations showing fewer points than they used to (3 loss values per iteration pre-Phase-36, 1 after) is the *intended* effect of the `epochs: 3 → 1` retune, not something this fix changes or could fix — verified directly against the same mission's pre-retune (3 points/iteration) vs post-retune (1 point/iteration) telemetry history.
