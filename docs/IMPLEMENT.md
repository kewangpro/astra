...
- [x] **28 new tests**: recipe hyperparameter regressions (`num_layers=8`, warm-start adapter, `iters`, `max_tokens`, `email_weight`), `LeadAgent` schema/prompt content, `test_bare_eval.py` (6 tests — pass-rate parsing, SSH command construction, missing-config/exception handling), `os.execv`-not-`subprocess.run` assertions for both templates, `get_sandbox_id` passthrough, and `remote_pid` recovery (reattach-when-alive, dead-when-gone, dead-when-host-unconfigured, terminate-reaches-the-real-pid) across `test_sandbox_manager.py` and `test_state_recovery.py`.

**Verified against a real live incident, not just tests:** launched an actual DPO mission end-to-end. First run: wrapper died mid-collection (root cause not fully determined — log rotated away, but likely coincided with an unrelated backend restart), orphaning the real `dpo_train.py` process — exactly the failure mode this phase's `os.execv` fix targets. Manually killed the orphan, relaunched with the fixed wrapper for a clean second attempt.

    Total: **657 tests** (648 unit + 9 integration).
