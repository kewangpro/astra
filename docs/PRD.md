...
- **Escalation persistence**: the consecutive-failed-pivot counter (`pivot_escalation_count`) is saved to the DB after every pivot and restored on server restart, so long-running missions correctly escalate even across process restarts or crashes. Reverting a regressed architecture restores the checkpoint but does not forgive the failed search attempt; the counter remains incremented so deep escalation and convergence remain reachable.
